...
for row in moma:
    begdata = row[3]
    enddata = row[4]
    row[3] = clean_and_convert(begdata)
    row[4] = clean_and_convert(enddata)
...
for element in test_data:
    stripped_test_data.append(strip_characters(element))
...
stripped_test_data = ['1912', '1929', '1913-1923',
                      ...
                      '1988', '2002', '1957-1959',
                      '1955', '1970', '1990-1999']
def process_date(strr):
    if '-' in strr:
        group = []
        group = strr.split('-')
        avg = round((int(group[0]) + int(group[1])) / 2)
        return avg
    else:
        return int(strr)
...
for element in stripped_test_data:
    processed_test_data.append(process_date(element))
